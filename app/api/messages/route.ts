    ...
    if (type === 'wall' || type === 'public') {
      ...
      const messagesWithReactions = messages.map(msg => {
        ...
        return {
          ...
          spotifyUrl: (msg as any).spotifyUrl,
          gifUrl: (msg as any).gifUrl
        }
      })
...
export async function POST(request: NextRequest) {
  ...
  let { content, toName, isPublic, spotifyUrl, gifUrl, theme } = body
  ...
  // Validate Spotify URL (OPTIONAL)
  if (spotifyUrl && !spotifyUrl.includes('spotify.com')) {
    spotifyUrl = null // Ignore invalid
  }
  ...
  try {
    ...
    // Style Generation
    let messageStyleStr: string

    if (theme === 'cyberpunk') {
      messageStyleStr = JSON.stringify({ 
        theme: 'cyberpunk',
        color: 'bg-black border-2 border-neon-pink text-neon-green', 
        rotation: 0 
      })
    } else if (theme === 'retro') {
      messageStyleStr = JSON.stringify({ 
        theme: 'retro',
        color: 'bg-gray-200 border-b-4 border-r-4 border-gray-800', 
        rotation: 0 
      })
    } else if (theme === 'vintage') {
      messageStyleStr = JSON.stringify({ 
        theme: 'vintage',
        color: 'bg-[#f4e4bc] border border-[#d4c49c]', 
        rotation: Math.floor(Math.random() * 3) - 1
      })
      ...
      messageStyleStr = JSON.stringify({ 
       ...
       messageStyleStr = JSON.stringify({ 
        ...
        messageStyleStr = JSON.stringify({ 
        theme: 'newspaper',
        color: 'bg-[#f4f4f4] border-double border-4 border-gray-400', 
        rotation: 0
      })
    } else if (theme === 'blueprint') {
        messageStyleStr = JSON.stringify({ 
        theme: 'blueprint',
        color: 'bg-[#1e3a8a] border-2 border-dashed border-blue-300', 
        rotation: 0
      })
    } else if (theme === 'galaxy') {
        messageStyleStr = JSON.stringify({ 
        theme: 'galaxy',
        color: 'bg-black border border-purple-500/50', 
        rotation: 0
      })
    } else {
      // Default Random Pastel
      ...
      messageStyleStr = JSON.stringify({ color: randomColor, rotation: randomRotation })
    }
